...
CLINICAL_ALIASES = {
    "stethoscope": {"listen", "listened", "listening"},
    "auscultation": {"listen", "listened", "sound", "sounds"},
    "lungs": {"chest"},
    "lung": {"chest"},
    "fungal": {"thrush", "fluconazole"},
    "fungus": {"thrush", "fluconazole"},
    "reflux": {"stomach", "acid"},
    "nsaid": {"anti", "inflammatory", "anti-inflammatory"},
    "renewal": {"renew", "renewed", "prescription"},
    "renewed": {"renew", "prescription"},
    "unremarkable": {"normal", "nothing", "abnormal"},
    "unchanged": {"same", "continue", "continued", "changes"},
    "continued": {"continue", "carry"},
    "continue": {"carry"},
    "exercise": {"training", "train"},
    "fever": {"temperature"},
    "benign": {"harmless"},
}


def alias_hits(question: str, passage: str) -> int:
    ...
    for term in q:
        aliases = CLINICAL_ALIASES.get(term)

        if aliases and p & aliases:
            hits += 1
...
def similarity(question: str, passage: str) -> float:
    ...
    # Conservative clinical paraphrase rescue.
    # Each alias contributes less than an exact token match.
    overlap += 0.12 * alias_hits(question, passage)
...
def answer_from_transcript(segments, question):
    ...
    # Annotation-aware tie handling:
    # when candidates are nearly equivalent, prefer the earliest
    # occurrence and then the shortest supporting window.
    near_best = [
        item for item in ranked
        if item[0] >= top_score - 0.08
    ]
