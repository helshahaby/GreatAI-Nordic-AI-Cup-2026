...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    if edge_angle is not None and edge_distance < 25.0:
        # Keep obstacle avoidance unless an observed fruit is already
        # directing us away from the threatening obstacle edge.
        fruit_moves_away = False

        if fruit is not None:
            fruit_angle_for_edge = float(fruit["angle"])
            separation = abs(
                wrap_angle(fruit_angle_for_edge - edge_angle)
            )

            fruit_moves_away = separation > math.pi / 2.0

        if not fruit_moves_away:
