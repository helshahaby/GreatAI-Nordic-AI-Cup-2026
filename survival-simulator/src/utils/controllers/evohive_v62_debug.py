...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    if edge_angle is not None and edge_distance < 25.0:
        # Normally preserve obstacle avoidance. If energy is becoming
        # dangerous and food is already very close, commit to the food
        # instead of repeatedly reversing between food and an edge.
        emergency_fruit_commit = (
            fruit is not None
            and energy_ratio <= 0.30
            and float(fruit["distance"]) < 30.0
        )

        if emergency_fruit_commit:
            print(
                "FRUITCOMMIT",
                f"id={agent_id}",
                f"age={age:.1f}",
                ...
                f"fruitD={float(fruit['distance']):.2f}",
                f"fruitA={float(fruit['angle']):.3f}",
                f"edgeD={edge_distance:.2f}",
                f"edgeA={edge_angle:.3f}",
            )

        if not emergency_fruit_commit:
            move_direction = wrap_angle(edge_angle + math.pi)
            move_distance = min(
                speed,
                max(speed * 0.40, edge_distance)
            )
