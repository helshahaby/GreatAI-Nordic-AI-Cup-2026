...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    # Diversify only during the vulnerable establishment phase.
    #
    # Early agents have not yet established a reliable food
    # route. Spreading them across visible resource regions
    # reduces the risk that the entire initial population follows
    # the same poor tree choice.
    #
    # Once established, return to V2's proven nearest-tree rule.
    early_search = age < 45.0 and energy_ratio < 0.40

    if visible_trees and early_search:
        visible_trees.sort(
            key=lambda o: float(o.get("distance", float("inf")))
        )

        candidates = visible_trees[:min(3, len(visible_trees))]
        tree = candidates[agent_id % len(candidates)]
