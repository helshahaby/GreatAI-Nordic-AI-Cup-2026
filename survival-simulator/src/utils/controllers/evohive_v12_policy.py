...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    # ---------------------------------------------------------
        ...
        # Low-energy agents must actively search through the
        # productive region instead of orbiting one tree forever.
        if energy_ratio < 0.28:
            move_distance = speed

            if tree_distance > 25.0:
                move_direction = tree_angle
            else:
                escape_side = 1.0 if agent_id % 2 == 0 else -1.0
                move_direction = wrap_angle(
                    tree_angle + escape_side * 1.15
                )
            ...
            # Healthy agents can afford a local search around
            # productive tree regions.
