...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    # Diversify resource search across visible trees.
    #
    # Instead of every agent choosing the same nearest tree,
    # sort visible trees by distance and deterministically assign
    # agents to different candidates.
    visible_trees = [
    ...
    if visible_trees:
        ...
        # Limit assignment to nearby candidates. This avoids
        # selecting a very distant tree simply because many trees
        # happen to be visible.
        candidates = visible_trees[:min(4, len(visible_trees))]
