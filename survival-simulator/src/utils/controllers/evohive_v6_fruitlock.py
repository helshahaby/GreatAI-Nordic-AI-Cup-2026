...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    if (
        ...
        and not (fruit is not None and float(fruit["distance"]) < 30.0)
    ):
        # Move away from the closest obstacle, except when food is already
        # within a short committed approach distance.
