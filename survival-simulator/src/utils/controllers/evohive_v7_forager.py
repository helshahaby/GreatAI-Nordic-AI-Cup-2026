...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    # During active fruit pursuit, trust the simulator's collision
    # resolution instead of overriding the foraging trajectory.
    # Explicit edge avoidance remains active for scouting/navigation.
    if (
        edge_angle is not None
        and edge_distance < 25.0
        and fruit is None
    ):
