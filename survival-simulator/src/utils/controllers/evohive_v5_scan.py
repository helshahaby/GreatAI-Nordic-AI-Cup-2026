...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    # ---------------------------------------------------------
    # ROLE 3A: EMERGENCY SENSOR SWEEP
    #
    # Fruit exists but can remain outside the vision cone.
    # When energy is becoming dangerous and no fruit is visible,
    # deliberately rotate the sensor heading while continuing
    # economical forward motion.
    # ---------------------------------------------------------
    elif energy_ratio < 0.18:
        scan_side = 1.0 if agent_id % 2 == 0 else -1.0

        if tree is not None:
            tree_angle = float(tree["angle"])
            tree_distance = float(tree["distance"])

            # Keep progressing through a resource region, but
            # scan much more aggressively than normal V2.
            move_direction = tree_angle
            move_distance = min(
                speed * 0.75,
                max(speed * 0.40, tree_distance - 20.0)
            )

            turn_angle = scan_side * 0.32

        else:
            # No known landmark: keep moving rather than spin
            # in place while sweeping the vision cone.
            move_direction = 0.0
