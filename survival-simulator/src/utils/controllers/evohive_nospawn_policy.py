...
def action_decision(observation_response: dict, rng: random.Random):
    ...
    # ---------------------------------------------------------
        ...
        move_distance = min(speed * 0.75, tree_distance)

        turn_angle = max(
            -0.08,
            min(0.08, tree_angle * 0.08)
        )

    # ---------------------------------------------------------
    # ROLE 4: EXPLORER
    # Cheap deterministic wandering instead of random sprinting.
    # ---------------------------------------------------------
    else:
        move_distance = speed * 0.65

        # Different agents naturally spread out.
        phase = (agent_id * 0.754877666) % (2.0 * math.pi)
        move_direction = 0.30 * math.sin(age * 0.08 + phase)

        turn_angle = 0.025 * math.sin(age * 0.04 + phase)
