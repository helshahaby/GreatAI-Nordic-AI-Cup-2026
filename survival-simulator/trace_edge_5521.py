...
from src.utils.controllers.evohive_v2_kite import edge_danger
...
while True:
    ...
    for agent, obs in zip(agents, obs_list):
        ...
        if 12.5 <= t <= 14.0:
            ed, ea = edge_danger(obs["observations"])
            fruits_dbg = [
            ...
            if fruit_dbg is not None:
                fd_dbg = float(fruit_dbg["distance"])
                fa_dbg = float(fruit_dbg["angle"])
                sep_dbg = abs(
                    (fa_dbg - ea + math.pi) % (2.0 * math.pi) - math.pi
                ) if ea is not None else float("nan")

                print(
                    f"EDGEDEBUG,{t:.1f},"
                    f"E={float(agent.energy):.2f},"
                    f"ratio={float(agent.energy)/float(agent.max_energy):.4f},"
                    f"fruitD={fd_dbg:.2f},"
                    f"fruitA={fa_dbg:.4f},"
                    f"edgeD={ed:.2f},"
                    f"edgeA={ea:.4f},"
